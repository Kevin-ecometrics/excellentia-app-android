...
9. **EditBatchActivity** (nueva, Fase 117) — editar una venta `AWAITING_APPROVAL`: reemplazo TOTAL de `items[]`, no un diff. Lista las líneas actuales (`item_edit_batch_row.xml`, un `MaterialCardView` por producto con `EditText` de cantidad/precio editables in-place y total recalculado en vivo) + botón "Add product" que reusa el patrón de búsqueda de `ConsignmentActivity.showManualEntryDialog()` (`dialog_manual_entry.xml`, `GET /api/products?search=` mientras se tipea, sin escaneo DataWedge en esta pantalla). Un producto sin `barcode` no se puede agregar (mismo motivo que el backend: `orders.barcode` es `NOT NULL`). "Save changes" lee las cantidades/precios en vivo de cada fila (no cachea estado — evita depender de que los `TextWatcher` mantengan todo sincronizado), arma `List<BatchItem>` y llama `POST .../edit`; éxito → `setResult(RESULT_OK)` + `finish()`.
   ...
   - **Fase 118 (2026-09-04) — `WarehouseRouteDetailActivity` tenía el bug opuesto: forzaba cantidad ENTERA para todo, incluido Lbs, al cargar un producto a una ruta.** No era un descuido — `route_items.quantity` era `INT` en la base (único eslabón de la cadena de Almacén que seguía así; `product_lots`/`route_item_lots`/`inventory_movements`/`route_returns` ya eran `DECIMAL` desde la Fase 112). Decidido con el usuario: ajustarlo a la misma lógica que ya rige en el resto del sistema (Lbs = peso real). Migración `route_items.quantity` INT → `DECIMAL(10,2)` (detalle completo, incluyendo el fix de serialización necesario en el backend, en `excellentia/CLAUDE.md`, sección "Fase 118"). `AddRouteItemRequest.quantity`/`RouteItemDto.quantity` pasaron de `Int` a `Double`; los 3 diálogos de cantidad de esta pantalla (`showQuantityDialog`, `showQuantityDialogForAvailable`, flujo FIFO) ahora usan `isLbsUnit()` para decidir decimal vs. entero, mismo criterio que `ReceivingActivity`/`ConsignmentActivity`. Nuevo helper compartido `formatQty()` en `data/Models.kt` (trimea ceros de relleno) usado acá y en `MyRouteDetailActivity` (vista de solo lectura del repartidor) para mostrar la cantidad sin que un Case/Unit entero se vea como "3.0".
   - **Fix (2026-09-04, misma sesión) — 2 pantallas más con el mismo bug, encontradas en una segunda revisión end-to-end.** A diferencia de la de arriba, estas dos necesitaron un campo nuevo en el backend (`unit`, agregado a `getExpectedReturns`/`listLots` — ver `excellentia/CLAUDE.md`, sección "Fase 118", sin migración SQL, solo una columna que ya existía en `products`): **`RouteReturnsActivity.kt`** — los 4 campos de cantidad al revisar devoluciones (Bueno/Dañado/Vencido/Transporter Damage, función `qtyField()`) eran siempre decimales sin mirar el tipo de producto; `RouteReturnExpectedDto` ganó `val unit: String? = null` y `qtyField()`/el botón "Sold out" ahora usan `isLbsUnit(exp.unit)`. **`InventoryMovementsActivity.kt`** — `showEditLotDialog()` (pestaña Disponible → Editar lote) tenía el mismo problema; `ProductLotDto` ganó `val unit: String? = null`, mismo fix. De paso, el badge de discrepancia del panel de reconciliación en `WarehouseRouteDetailActivity` (`renderReconciliation()`) redondeaba a 0 decimales (`%+.0f`) mientras la línea de arriba mostraba precisión completa — ajustado a `%+.2f`.
   - **Fix (2026-09-04, mismo día, reportado por el usuario) — `btnSaveReturns` (`activity_route_returns.xml`) y `btnSaveReceipt` (`activity_receiving.xml`) mismo bug de contraste ya visto varias veces esta sesión.** Los dos usan `app:backgroundTint="@color/primary"` (verde) sin `strokeColor`, sobre una pantalla cuyo fondo (`@color/background`) es el mismo verde — el botón se veía sin borde, indistinguible del fondo ("el fondo del botón es el mismo que el fondo"). Otros botones idénticos en la app (`activity_pre_order_detail.xml`, `activity_settings.xml`, `activity_create_pre_order.xml`) ya tenían `app:strokeColor="@color/white"` — estos dos se habían quedado afuera cuando se estableció ese patrón. Agregado `strokeColor="@color/white"` + `strokeWidth="1dp"` a los dos.
10. **SettingsActivity** — backend URL, offline mode, **impresora Bluetooth** (lista dispositivos emparejados, botón probar), cerrar sesión.
11. **PreOrderListActivity** — lista pre-órdenes del servidor. Chips: **Pendientes** (DRAFT, default) / **Convertidas** (CONVERTED) / **Canceladas** (CANCELLED) / **Todas**. FAB blanco "Nueva pre-orden" → CreatePreOrderActivity. Click card → PreOrderDetailActivity. Empty state dinámico por chip. Fechas formateadas con `formatDate()` (soporta ISO con ms, sin ms, y `yyyy-MM-dd`).
12. **CreatePreOrderActivity** — crear pre-orden: seleccionar cliente (CustomerPickerActivity launcher), DatePicker para fecha de entrega, notas, agregar ítems por escaneo DataWedge o búsqueda por nombre. **Solo selección de producto** (Fase 87) — sin precio/peso/cantidad, el ítem se agrega directo (barcode+nombre) sin abrir `ProductDetailActivity`; re-escanear un barcode ya agregado muestra "ya está en la lista" en vez de duplicar. Guarda via `POST /api/preorders`. Pre-llena cliente activo si hay uno seleccionado.
13. **PreOrderDetailActivity** — detalle de pre-orden. Para DRAFT/CONFIRMED, antes de convertir hay que **detallar cada producto** (Fase 87): cada ítem sin detallar muestra botón "Detallar" que pide el producto fresco del catálogo (`getProductByBarcode`) y relanza el mismo stepper de `ProductDetailActivity` (peso/case/precio) que antes se usaba al crear; "Convertir" queda deshabilitado hasta que todos los ítems tengan su detalle (contador "X de Y detallados" en el header de la sección). Flujo de conversión idéntico a `CurrentOrderActivity`: Firma → Artículos dañados → Método de pago → Check impresora → `POST /api/preorders/:id/convert` (manda los ítems ya detallados en el body) → `OrderSuccessActivity`. Loading overlay con 3 pasos. Botones por estado: DRAFT/CONFIRMED → "Convertir" + "Cancelar"; CONVERTED → "Reusar pre-orden" (crea nueva DRAFT con los mismos productos, **sin** el precio/detalle ya finalizado — la nueva pre-orden también debe detallarse el día que le toque) + "Ver en historial"; CANCELLED → sin botones. Fechas formateadas con `formatDate()`. Recibe `pre_order_id` por intent.
14. **ClientHistoryActivity** — historial de pedidos de un cliente específico. Header con nombre + resumen (pedidos, total). Lista de batch cards (reutiliza `item_batch_header.xml`). Click → carga orders del cliente y abre TicketDetailActivity. Recibe `customer_id` + `customer_name` por intent. Accesible desde: botón "Historial" en tarjeta de cliente activo de MainActivity, y long-press en CustomerPickerActivity.
15. **IssueCreditActivity** — agenda crédito para un cliente **sin venta asociada** (ej. producto dañado/caducado detectado sin que se vaya a vender). Accesible desde botón "Agregar crédito" en MainActivity (mismo estilo que "Pre-órdenes"). Elegir cliente (CustomerPickerActivity) → agregar productos por escaneo DataWedge o búsqueda manual → diálogo de cantidad dañada/caducada por producto (decimal si el producto es Lbs — peso real dañado —, entero para Case/Unit/Bucket) → lista editable con estimado de crédito en pantalla → `POST /api/credits/issue`. Sin firma ni impresión — acción interna simple. No genera ningún documento en QuickBooks; el crédito queda disponible de inmediato en el balance del cliente (mismo ledger `credit_transactions` que consulta "Apply Credit" al finalizar un pedido normal).
